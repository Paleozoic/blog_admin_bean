...
public class Jackson2JsonRedisSerializer<T> implements RedisSerializer<T> {
	...
	public List<T> deserializeList(String str) throws SerializationException {
    	 
        if (SerializationUtils.isEmpty(str)) {
            return null;
        }
        try {
    		JavaType t = this.objectMapper.getTypeFactory().constructParametrizedType(List.class, List.class,javaType); 
    		return (List<T>) this.objectMapper.readValue(str, t); 
        } catch (Exception ex) {
            throw new SerializationException("Could not read JSON: " + ex.getMessage(), ex);
        }
    }
